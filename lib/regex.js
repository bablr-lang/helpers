...
export const grammar = new Grammar({
  productions: productions({
    RegExpLiteral(p) {
      p.eat('/');
      p.spans.push({ type: 'Expression', guard: '/' });
      const pattern = p.eatProduction('Pattern');
      p.spans.pop();
      p.eat('/');
      let flags = p.eatProduction('Flags');
    ...
    Alternatives(p) {
      ...
      while (p.eatMatch('|') && !p.done) {
        p.eatProduction('Alternative');
      }
    },

    Alternative(p) {
      // TODO make this work with guards
      while (!p.done) p.eatProduction('Element');
    },
    ...
    CharacterClass(p) {
      p.eat('[');
      p.spans.push({ type: 'CharacterClass', guard: ']' });
    ...
    Flags(p) {
      ...
      return flags; // TODO return object
    },
  }),
