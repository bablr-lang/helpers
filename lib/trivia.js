...
const lookbehind = (context, s) => {
  let token = s.result;
  while (token && ['OpenNode', 'CloseNode', 'OpenFragment', 'ReferenceTag'].includes(token.type)) {
      ...
      try {
        while (!co.done) {
          ...
          switch (verb) {
            ...
            case 'match':
            case 'guard': {
              if (matcher.type && !matcher.flags.trivia) {
                ...
                if (isString(matcher) || matcher.type === 'Pattern') {
                  isTerminal = true;
                }
              }
